...
const calculateMaxValue = (data, primaryKey, secondaryKey, showSecondary) => {
    ...
    const maxPrimary = Math.max(
        ...data.map((item) => Number(item[primaryKey]) || 0)
    );
    const maxSecondary = showSecondary
        ? Math.max(...data.map((item) => Number(item[secondaryKey]) || 0))
        : 0;
...
const BarChartComponent = ({
    ...
    useEffect(() => {
        ...
        // Crear datos base para los meses a mostra
        ...
        const processedData = monthsToShow.map((monthNum) => {
            ...
            const secondaryValue = showSecondaryBar
                ? Number(monthData[dataKeys.secondary])
                : 0;

            return {
                ...baseMonth,
                [dataKeys.primary]:
                    !isNaN(primaryValue) && primaryValue > 0
                        ? primaryValue
                        : null,
                [dataKeys.secondary]:
                    showSecondaryBar &&
